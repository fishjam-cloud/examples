...
export class FishjamAudioInterface extends AudioInterface {
    ...
    private _startAudioInput(): void {
        ...
        this.inputInterval = setInterval(() => {
            ...
            // here data from VADstream event notifier(audio-streaming-orchestrator.ts) will be bassed to inputCallback
            const vadChunk = Buffer.alloc(chunkSize); // This should be replaced with actual VAD audio data
            this.inputCallback(vadChunk);
...
/**
 * Creates a new FishjamAudioInterface instance.
 *
 * @returns A new FishjamAudioInterface instance
 */
export function createFishjamAudioInterface(): FishjamAudioInterface {
    return new FishjamAudioInterface();
}