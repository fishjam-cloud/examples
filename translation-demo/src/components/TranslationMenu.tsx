...
export const TranslationMenu = ({
  ...
  const selectedTranslation = selectedKey
    ? translations.find((option) => option.key === selectedKey)
    : undefined;
  const selectedTranslationPending =
    selectedTranslation?.status === "requestable";

  const activeTranslations = useMemo(
    () => translations.filter((option) => option.status === "active"),
    [translations],
  );
    ...
    >
      <PopoverTrigger asChild>
        ...
        >
          ...
          {(selectedTranslationPending || audioLoading) && (
      ...
      >
        ...
        >
          ...
          {!selectedTranslation && (
            <Check size={14} className="shrink-0 text-stone-900" />
          )}
        </button>
